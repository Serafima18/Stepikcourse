...
if mode == 'SAVE':  # SAVE TO DATA
    ...
    idd = course['id']
    course = {key: course[key] for key in
              ['title', 'summary', 'course_format', 'language', 'requirements', 'workload', 'is_public', 'description',
    ...
    for row in data:
        ...
        if cross_domain:  # hack for attachements, only works for when source domain is stepik.org
            row[2]['block']['text'] = row[2]['block']['text'].replace('href="/media/attachments/',
                                                                      'href="https://stepik.org/media/attachments/')
